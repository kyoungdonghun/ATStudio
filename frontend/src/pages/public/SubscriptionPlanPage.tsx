...
function getTierFeatures(name: string, plan: SubscriptionPlan): TierFeatures {
  ...
  switch (name) {
    case 'STANDARD':
      return {
        features: [...base, { text: '\uC6B0\uC120 \uC9C0\uC6D0', included: false }],
        btnLabel: '\uC2DC\uC791\uD558\uAE30',
        btnVariant: 'ghost',
      };
    case 'DELUXE':
...
export default function SubscriptionPlanPage() {
  ...
  const compareRows: Array<{ label: string; values: string[] }> = [
    ...
    {
      label: '\uC6B0\uC120 \uC9C0\uC6D0',
      values: sortedPlans.map((p) => (p.name === 'PREMIUM' ? '\u2713' : '\u2717')),
    },
    {
      label: '\uAD6C\uB3C5 \uCDE8\uC18C',
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.billingToggle}>
        <span className={`${styles.toggleLabel} ${!isYearly ? styles.toggleOn : ''}`}>
          {'\uC6D4\uAC04 \uACB0\uC81C'}
        </span>
        <button
        ...
        {isYearly && <span className={styles.badgeSave}>{'2\uAC1C\uC6D4 \uBB34\uB8CC'}</span>}
      </div>
      ...
      {mySub && (mySub.status === 'ACTIVE' || mySub.status === 'CANCELLED') && (
        <div className={styles.currentSubBanner}>
          ...
          {' \uD50C\uB79C\uC744 \uAD6C\uB3C5 \uC911\uC785\uB2C8\uB2E4.'}
          <button className={styles.manageLink} onClick={() => navigate('/subscriptions/manage')}>
            {'\uAD6C\uB3C5 \uAD00\uB9AC \u2192'}
          </button>
        </div>
      )}
          ...
          return (
            <div key={plan.id} className={cardClass}>
              ...
              <div
                ...
                style={isMiddle ? { color: 'var(--accent)' } : undefined}
              >
              ...
              <hr
                ...
                style={isMiddle ? { borderColor: 'var(--accent-border)' } : undefined}
              />
              <ul className={styles.features}>
                {tier.features.map((f) => (
                  <li key={f.text} className={f.included ? '' : styles.dim}>
      ...
      <section className={styles.faqSection}>
        ...
        {FAQ_ITEMS.map((item, idx) => (
          ...
          >
            ...
            {openFaq === idx && <div className={styles.faqA}>{item.answer}</div>}
          </div>
        ))}
      </section>
    </div>
