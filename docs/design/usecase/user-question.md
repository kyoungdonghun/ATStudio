...
2. Backend checks inquiry viewing permissions before resolving the attachment's
   private Resource.
3. The controller returns an authorized `StreamingResponseBody`: it transfers
   the service Resource without a controller-sized intermediate byte array and
   closes the input stream after transfer. The response uses UTF-8 encoded
   attachment disposition, `application/octet-stream`, `no-store, private`,
   `Pragma: no-cache`, `nosniff`, sandbox CSP, and `Accept-Ranges: none`.
4. Frontend triggers the browser download only if the initiating authenticated
   owner, route, and detail projection are still current and the canonical
   binary result has a non-empty Blob. A safe server filename and content type
   take precedence; an unsafe response filename selects the stable fallback.
   Route, owner, projection replacement, or unmount aborts and retires the
   request.
...
- A validated attachment download is initiated in the browser. The server does
  not persist client byte-delivery completion as a result of this request.
