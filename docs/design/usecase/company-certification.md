...
3. Before submission, the admin UI claims the selected certification as an
   immutable review owner. Close, cancel, duplicate review, and another row's
   detail action are rejected until the submitted review and its authoritative
   refreshes settle.
4. Backend locks the owning user row and then the target certification row, validates the transition and conditional reason before any state/audit mutation, and updates the company_certifications record.
...
5. Backend records reviewer identity, timestamp, and from/to statuses without storing document contents or paths, then returns the updated certification.
6. On success, the UI refreshes the same certification detail and then the
   canonical review list before releasing ownership. A failure remains attached
   to the original certification and does not update another row's detail.
