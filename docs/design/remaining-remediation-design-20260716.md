...
WI-011 recorded a dated dependency checkpoint in which the development branch resolved Vite 6.4.3 and its then-current production-only and unfiltered audits reported zero findings. That result remains historical evidence rather than a current dependency claim. The 2026-07-24 registry resolves `react-router-dom@6.30.4` with `react-router@6.30.4`, and the current npm advisory database reports two moderate vulnerable package records containing three advisory records. [WI-20260724-ATS-021](../../deliverables/agent/WI-20260724-ATS-021-evidence-pack.md) found no exploitable open-redirect/XSS call site in the current ATStudio route graph and found the SSR hydration constructor-injection advisory unreachable in the current client-only architecture. Those reachability conclusions do not remove the vulnerable package status. The current V1 rehearsal is conditionally acceptable only as a non-production exercise; production readiness requires separate approval and completion of a controlled migration to `react-router-dom@7.18.1` and its matching `react-router` dependency with the full frontend gate suite.
