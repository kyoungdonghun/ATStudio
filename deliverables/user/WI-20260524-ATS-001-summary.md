...
- 현재 자동화 경계도 명시했다. 스케줄러는 WARN 로그를 남기고 admin endpoint는 현재 상태를 조회하지만, 별도 incident 저장/운영자 알림은 아직 없다.
...
- DB 스키마 변경은 하지 않았다.
- 운영자가 사용자 신고 없이 더 확실히 캐치하려면 persistent incident storage와 operator notification이 후속 작업으로 필요하다.
...
- Reconciliation incident 저장 및 운영자 알림.
- 실제 환불/취소 자동화.
