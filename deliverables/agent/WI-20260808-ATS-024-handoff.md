[WI HEADER]
WI ID: WI-20260808-ATS-024
REQ: REQ-20260808-ATS-004
Agent: qa-fe
Depends On: WI-20260808-ATS-022
Blocks: WI-20260808-ATS-028~030
[WI SUMMARY]
Why: 변경된 프론트 기능의 Vitest 전체 테스트와 coverage를 독립 검증한다.
Scope (in/out): 전체 Vitest와 coverage, 신규 UI 상태·race·URL·접근성 회귀. 코드 수정 없이 결함 보고.
DoD: 테스트 실패 0; 기존 coverage 임계값 통과; 핵심 경로 근거 기록.
Constraints/Forbidden: 현재 브랜치에서만 작업한다. 무관 파일·기존 ZIP·비밀값을 수정하지 않는다. 파일 삭제, 스키마 변경, 실제 데이터 변경, 외부 결제 호출은 승인 없이 금지한다.

[ACCEPTANCE CRITERIA]
Functional:
- [ ] 테스트 실패 0; 기존 coverage 임계값 통과; 핵심 경로 근거 기록.
Performance:
...
[OUTPUT CONTRACT]
User-facing -> deliverables/user/WI-20260808-ATS-024-summary.md
Agent-facing -> deliverables/agent/WI-20260808-ATS-024-evidence-pack.md
Handoff Packet -> deliverables/agent/WI-20260808-ATS-024-handoff.md
